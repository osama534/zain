...
$sql = "SELECT * FROM canceled_companys WHERE id = $contractId";
...
$insertSql = "INSERT INTO company (
    contract_number, company_name, phone_number, activation_date, address, change_every, area, contract_notes, 
    ...
    '{$conn->real_escape_string($contract['company_name'])}',
...
if (!$insertResult) {
    die('Error inserting into company table: ' . $conn->error);
}

// Delete the contract from canceled_contracts
$deleteSql = "DELETE FROM canceled_companys WHERE id = $contractId";
$deleteResult = $conn->query($deleteSql);

if (!$deleteResult) {
    die('Error deleting from canceled_companys table: ' . $conn->error);
}

// Redirect to the same page with a success message
header('Location: activate_canceled.php?status=success');
exit();
?>
